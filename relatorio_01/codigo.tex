...
\lstinputlisting[label=code:formExact,linerange={49-93},language=C,caption=formExact()]{codigo/poisson_fd_2d.c}

\subsection{Resolução do sistema de equações}
O Código \ref{code:solve}  mostra os vários objetos necessários para resolver o problema de Poisson, um \verb|DM|, um \verb|Mat| e três \verb|Vec|. Um objeto \verb|DM| pode calcular tamanhos de matriz e vetor a partir das dimensões da malha, o que acontece quando chamamos \verb|DMCreateMatrix()| e \verb|DMCreateGlobalVector()| para criar objetos \verb|Mat| e \verb|Vec|, respectivamente. Então chamamos as funções mostradas nos Códigos \ref{code:formMatrix} e \ref{code:formExact} para montar a matriz e vetores.

\lstinputlisting[label=code:solve,linerange={97-148},language=C,caption=formExact()]{codigo/poisson_fd_2d.c}

O sistema linear é resolvido por um objeto de solução linear KSP. No Código \ref{code:solve} criamos e contamos sobre Mat A através de uma chamada para KSPSetOperators (). Existem duas formas de utilização de A, nomeadamente a matriz do sistema e o "material" a partir do qual o preconicionador é construído, e isto explica por que A aparece como dois argumentos de KSPSetOperators (). Também chamamos KSPSetFromOptions () para que possamos alterar o tipo de KSP no tempo de execução (ilustrado abaixo). O sistema é resolvido chamando KSPSolve (). Então calculamos e relatamos o erro numérico k u - u ex k. Finalmente, destruímos objetos e chamamos PetscFinalize ().