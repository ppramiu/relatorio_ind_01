...
Como exemplo, a seguir será construído o sistema de equações para o caso da malha representada na Figura \ref{fig:poisson_grid_2d_4x3}, em que $m_x=4$ e $m_y=3$, o que resulta em $h_x=1/3$ e $h_y=1/2$. Somente os pontos com índice global $k=5$ e $k=6$ não são condições de contorno e o sistema linear é dado por:
\begin{equation}
  \label{eq:sistema_01}
    \left[
...
\begin{flushleft}
\verb|user@user-G73Sw:~$ ./poisson_fd_2d -da_grid_x 5 -da_grid_y 7 -dm_view draw -draw_pause 5|\\
\end{flushleft}
irá plotar a Figura \ref{fig:grid_xwindow_poisson}.
\begin{figure}[H] \centering
 \includegraphics[width=0.5\textwidth]{figuras/grid_xwindow_poisson}
 \caption{Malha computacional.}
 \label{fig:grid_xwindow_poisson}
\end{figure}
Outra visualização interessante é obtida com o comando:
\begin{flushleft}
\verb|user@user-G73Sw:~$ ./poisson_fd_2d -da_grid_x 5 -da_grid_y 7 -a_mat_view draw -draw_pause 5|\\
\end{flushleft}
que apresenta a estrutura da matriz A, conforme Figura \ref{fig:matrix_struct_xwindow}.
\begin{figure}[H] \centering
 \includegraphics[width=0.5\textwidth]{figuras/matrix_struct_xwindow}
