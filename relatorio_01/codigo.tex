...
Como exemplo da aplicação das aproximações por diferenças, utiliza-se a função $u(x) = \sin(x)$ e o ponto $\bar{x}=1$. Sabe-se que $u'(1) = \cos(1) = 0.5403023$. O código \ref{code:example_1_1_leveque} ilustra uma simples implementação serial para este exemplo empregadn equação de diferenças avançadas, atrasadas e centradas, denotadas na saída do console respectivamente por, D+, D- e D0. Foram utilizados nesse exemplo diferentes espaçamentos $h$.
